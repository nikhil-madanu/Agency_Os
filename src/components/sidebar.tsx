...
export function Sidebar({ activeView, onNavigate, isCollapsed = false, onToggle }: SidebarProps) {
  return (
    <aside className={cn(
      "hidden lg:flex fixed left-2 top-3 bottom-3 z-40 bg-white flex-col py-4 xl:py-5 overflow-visible rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100/80 transition-all duration-300 ease-in-out",
      isCollapsed ? "w-[80px] px-3" : "w-[230px] xl:w-[280px] px-4 xl:px-5"
    )}>
      <div className="flex flex-col flex-1 min-h-max relative">
        ...
        {/* Logo Section */}
        <div className={cn("flex items-center mb-4 xl:mb-5 transition-all", isCollapsed ? "justify-center mt-6 px-0" : "gap-3 px-1 xl:px-2")}>
          <div className={cn("flex flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50/50 shadow-sm border border-blue-100/50", isCollapsed ? "h-[40px] w-[40px]" : "h-[42px] w-[42px] xl:h-[46px] xl:w-[46px]")}>
            <img 
        ...
        <div className={cn("group relative flex items-center hover:bg-slate-50 transition-colors cursor-pointer border border-transparent hover:border-slate-100", isCollapsed ? "justify-center p-2 mb-4 rounded-xl" : "gap-3 px-2 mb-4 xl:mb-5 p-2 rounded-2xl")}>
          <img 
            src="https://i.pravatar.cc/150?img=11" 
            alt="Arjun Mehta" 
            className={cn("rounded-full object-cover ring-2 ring-white shadow-sm flex-shrink-0", isCollapsed ? "w-[36px] h-[36px]" : "w-[38px] h-[38px] xl:w-[42px] xl:h-[42px]")} 
          />
          {!isCollapsed && (
        ...
        {/* Main Navigation */}
        <nav className="flex flex-col gap-0.5 xl:gap-2 flex-1 mt-0">
          {mainNavItems.map((item) => (
            <a
              ...
              className={cn(
                ...
                isCollapsed ? "justify-center p-3 rounded-xl" : "gap-3 px-3 xl:px-4 py-2 xl:py-3 rounded-2xl",
      ...
      {/* Bottom Navigation */}
      <div className="border-t border-slate-100/80 pt-3 xl:pt-4 mt-auto flex flex-col gap-0.5 xl:gap-2 relative z-50">
        <a href="#" className={cn("group relative flex items-center font-medium text-slate-500 hover:bg-slate-50 hover:text-slate-900 border border-transparent transition-all duration-300", isCollapsed ? "justify-center p-3 rounded-xl relative" : "justify-between px-3 xl:px-4 py-2 xl:py-3 rounded-2xl text-[13.5px] xl:text-[14.5px]")}>
        ...
        <a href="#" className={cn("group relative flex items-center font-medium text-slate-500 hover:bg-slate-50 hover:text-slate-900 border border-transparent transition-all duration-300", isCollapsed ? "justify-center p-3 rounded-xl" : "gap-3 px-3 xl:px-4 py-2 xl:py-3 rounded-2xl text-[13.5px] xl:text-[14.5px]")}>
        ...
        <button className={cn("group relative flex items-center font-medium text-slate-500 hover:bg-red-50 hover:text-red-700 border border-transparent transition-all duration-300 text-left w-full", isCollapsed ? "justify-center p-3 rounded-xl" : "gap-3 px-3 xl:px-4 py-2 xl:py-3 rounded-2xl text-[13.5px] xl:text-[14.5px]")}>
